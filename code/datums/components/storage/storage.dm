#define STORAGE_COMPONENT_GET_REAL_LOCATION var/atom/real_location = STORAGE_COMPONENT_ACCESS_REAL_LOCATION
#define STORAGE_COMPONENT_ACCESS_REAL_LOCATION real_location()
#define STORAGE_COMPONENT_GET_PARENT_ATOM var/atom/parent = src.parent
#define STORAGE_COMPONENT_GET_MASTER_STORAGE var/datum/component/storage/concrete/master = master()
...
/datum/component/storage
	...
	var/list/mob/is_using = list()							//lazy list of mobs looking at the contents of this storage.
...
/datum/component/storage/OnTransfer()
	update_actions()
...
/datum/component/storage/proc/attack_obj(obj/O, mob/M)
	. = FALSE
...
/datum/component/storage/proc/standard_orient_objs(rows, cols, list/obj/item/display_contents)
	var/cx = 4
	var/cy = 2+rows
	boxes.screen_loc = "4:16,2:16 to [4+cols]:16,[2+rows]:16"

	if(display_numerical_stacking)
		for(var/type in display_contents)
			var/datum/numbered_display/ND = display_contents[type]
...
/datum/component/storage/proc/signal_take_type(type, atom/destination, amount = INFINITY, check_adjacent = FALSE, force = FALSE, mob/user)
	if(!force)
		if(check_adjacent)
			if(user)
	...
	for(var/i in taking)
		...
		remove_from_storage(AM, destination)
	return TRUE
...
/datum/component/storage/proc/signal_take_obj(atom/movable/AM, new_loc, force = FALSE)
	if(!(AM in STORAGE_COMPONENT_ACCESS_REAL_LOCATION))
		return FALSE
...
/datum/component/storage/proc/on_alt_click(mob/user)
	if(user.incapacitated() || user.CanReachStorage(src))
		return
	var/obj/item/I = locate() in STORAGE_COMPONENT_ACCESS_REAL_LOCATION
	if(!I)
		return
...
/datum/component/storage/proc/gather_mode_switch(mob/user)
	...
	switch(collection_mode)
		if(COLLECT_SAME)
			to_chat(user, "[src] now picks up all items of a single type at once.")

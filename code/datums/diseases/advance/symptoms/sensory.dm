...
/datum/symptom/mind_restoration/Activate(var/datum/disease/advance/A)
	...
	if(A.stage >= 3)
		...
		if(purge_alcohol)
			Mreagents.remove_all_type(/datum/reagent/consumable/ethanol, 3)
			if(ishuman(M))
	...
	if(A.stage >= 4)
		M.drowsyness = max(0, M.drowsyness - 2)
		if(Mreagents.has_reagent("mindbreaker"))
			Mreagents.remove_reagent("mindbreaker", 5)
		if(Mreagents.has_reagent("histamine"))
			Mreagents.remove_reagent("histamine", 5)

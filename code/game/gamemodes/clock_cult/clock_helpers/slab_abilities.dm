...
/obj/effect/proc_holder/slab/compromise/InterceptClickOn(mob/living/caller, params, atom/target)
	...
	if(isliving(target) && (target in view(7, get_turf(ranged_ability_user))))
		...
		if(!totaldamage && (!Lreagents || !Lreagents.has_reagent("holywater")))
		...
		var/has_holy_water = (Lreagents && Lreagents.has_reagent("holywater"))
		...
		if(has_holy_water)
			Lreagents.remove_reagent("holywater", 1000)

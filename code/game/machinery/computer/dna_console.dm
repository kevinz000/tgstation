	...
	if(connected && connected.is_operational())
		if(connected.occupant)	//set occupant_status message
			...
			if(viable_occupant.has_dna() && (!(RADIMMUNE in viable_occupant.dna.species.species_traits)) && (!(viable_occupant.disabilities & NOCLONE) || (connected.scan_level == 3))) //occupant is viable for dna modification
				...
				var/rejuvenators = viable_occupantreagents.get_reagent_amount("potass_iodide")
...
/obj/machinery/computer/scan_consolenew/Topic(href, href_list)
	...
	var/last_change
	GET_COMPONENT_FROM(OR, /datum/component/reagents, viable_occupant)
	switch(href_list["task"])
		...
		if("rejuv")
			if(OR)
				var/potassiodide_amount = OR.get_reagent_amount("potass_iodide")
				var/can_add = max(min(REJUVENATORS_MAX - potassiodide_amount, REJUVENATORS_INJECT), 0)
				OR.add_reagent("potass_iodide", can_add)
		if("setbufferlabel")

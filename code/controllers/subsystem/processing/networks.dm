PROCESSING_SUBSYSTEM_DEF(networks)
	...
	var/datum/exonet/station/station_network
...
/datum/controller/subsystem/processing/networks/proc/register_network(datum/exonet/network)
	if(!networks_by_id[network.network_id])
		networks_by_id[network.network_id] = network
		return TRUE
	return FALSE

/datum/controller/subsystem/processing/networks/proc/unregister_network(datum/exonet/network)
	networks_by_id -= network.network_id
	return TRUE

/datum/controller/subsystem/processing/networks/proc/register_interface(datum/component/exonet_interface/D)
	if(!interfaces_by_id[D.hardware_id])
		interfaces_by_id[D.hardware_id] = D
		return TRUE
	return FALSE

/datum/controller/subsystem/processing/networks/proc/unregister_interface(datum/component/exonet_interface/D)

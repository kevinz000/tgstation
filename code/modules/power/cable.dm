...
//Finds connected cables
/obj/structure/cable/proc/powernet_expansion()
	. = list()
	if(d2)				//Find connected cables for second direction
		var/turf/T = get_step(src, d2)
		var/other_dir = turn(d2, 180)
		for(var/obj/structure/cable/C in T)
			if((C.d1 == other_dir) || (C.d2 == other_dir))
				. |= C
	if(d1)				//Find connected cables for first direction
		var/turf/T = get_step(src, d1)
		var/other_dir = turn(d1, 180)
		for(var/obj/structure/cable/C in T)
			if((C.d1 == other_dir) || (C.d2 == other_dir))
				. |= C
	else				//No first connection, find wire knots on our turf

...
/datum/effect_system/smoke_spread/chem/smoke_machine/set_up(datum/component/reagents/carry, setting = 3, efficiency = 10, loc)
	amount = setting
...
/obj/machinery/smoke_machine/attackby(obj/item/I, mob/user, params)
	...
	if(istype(I, /obj/item/reagent_containers) && I.is_open_container())
		...
		var/units = RCreagents.trans_to(src, RC.amount_per_transfer_from_this)
		if(units)
			to_chat(user, "<span class='notice'>You transfer [units] units of the solution to [src].</span>")
			add_logs(usr, src, "has added [english_list(RCreagents.reagent_list)] to [src]")
			return

...
/datum/reagent/blob/regenerative_materia/reaction_mob(mob/living/M, method=TOUCH, reac_volume, show_message, touch_protection, mob/camera/blob/O)
	...
	if(Mreagents)
		Mreagents.add_reagent("regenerative_materia", 0.2*reac_volume)
		Mreagents.add_reagent("spore", 0.2*reac_volume)
...
/datum/reagent/blob/cryogenic_poison/reaction_mob(mob/living/M, method=TOUCH, reac_volume, show_message, touch_protection, mob/camera/blob/O)
	...
	GET_COMPONENT_FROM(Mreagents, /datum/component/reagents, M)
	if(Mreagents)
		Mreagents.add_reagent("frostoil", 0.3*reac_volume)
		Mreagents.add_reagent("ice", 0.3*reac_volume)
		Mreagents.add_reagent("cryogenic_poison", 0.3*reac_volume)

...
#define FLIGHT_DO_NOT_MOVE 0
#define FLIGHT_PASS_TO_FLIGHTSUIT 1

/datum/flight_momentum
	name = "flight momentum handler"

	var/obj/item/clothing/suit/space/hardsuit/flightsuit/suit = null

	var/move_mode = FLIGHT_DO_NOT_MOVE
	...
	var/momentum_impact_coeff = 0.5
	var/momentum_impact_loss = 50
	var/momentum_crash_coeff = 0.8
...
/datum/flight_momentum/proc/transferMove(dir, amount)
	switch(move_mode)
		if(FLIGHT_DO_NOT_MOVE)
			return FALSE
		if(FLIGHT_PASS_TO_FLIGHTSUIT)
			suit.driftMove(dir, amount)

/datum/flight_momentum/proc/get_decay()
	switch(move_mode)
		if(FLIGHT_DO_NOT_MOVE)
			return 500
		if(FLIGHT_PASS_TO_FLIGHTSUIT)
...
/datum/flight_momentum/proc/get_power()
	switch(move_mode)
		if(FLIGHT_DO_NOT_MOVE)
			return 0
		if(FLIGHT_PASS_TO_FLIGHTSUIT)
...
/datum/flight_momentum/proc/adjust(amountx, amounty, reduce_amount_total = 0)
...
/datum/flight_momentum/proc/onMove(dir)
	if(!flight)
		return FALSE
	switch(dir)
		if(NORTH)
			adjust_momentum(0, flightpower)
		if(SOUTH)
			adjust_momentum(0, -flightpower)
		if(EAST)
			adjust_momentum(flightpower, 0)
		if(WEST)
			adjust_momentum(-flightpower, 0)

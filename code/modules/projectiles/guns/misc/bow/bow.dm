...
/obj/item/gun/ballistic/bow/proc/process_zoom()
	if(!check_user() || !zooming)
		return
	current_user.client.change_view(round((getviewsize(CONFIG_GET(string/default_view)) - 1)[1] / 2 + zoom_view_increase))
...
/obj/item/gun/ballistic/bow/attackby(obj/item/I, mob/user)
	if(user.temporarilyRemoveItemFromInventory(I))
		if(!magazine.give_round(I, FALSE))
			I.forceMove(user.drop_location())
			user.put_in_active_hand(I)
		else
			to_chat(user, "<span class='notice'>You put [I] onto [src]'s [magazine].</span>")
	return ..()

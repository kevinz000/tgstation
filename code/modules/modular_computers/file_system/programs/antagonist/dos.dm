...
/datum/computer_file/program/exonet_dos/ui_act(action, params)
	...
	switch(action)
		if("PRG_target_relay")
			for(var/obj/machinery/exonet_relay/R in GLOB.exonet_global.relays)
		...
		if("PRG_execute")
			if(target)
				...
				if(GLOB.exonet_global.intrusion_detection_enabled)
					var/obj/item/computer_hardware/network_card/network_card = computer.all_components[MC_NET]
					GLOB.exonet_global.add_log("IDS WARNING - Excess traffic flood targeting relay [target.uid] detected from device: [network_card.get_network_tag()]")
					GLOB.exonet_global.intrusion_detection_alarm = 1
			return 1

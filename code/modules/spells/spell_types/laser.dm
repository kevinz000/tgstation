...
*/
	//Misc icons
	var/base_button_icon_state = "laser_mouse_target"
	...
	//Beam
	var/beam_size = 1
	var/image/beam_icon_leading
	var/image/beam_icon_tracer
	var/image/beam_icon_trailing
...
/obj/effect/proc_holder/spell/laser/Initialize()
	..()
	beam_icon_tracer = icon('icons/effects/96x96.dmi',icon_state = "laser_tracer")
	beam_icon_leading = icon('icons/effects/96x96.dmi',icon_state = "laser_leading")
...
/obj/effect/proc_holder/spell/laser/proc/handle_firing()
	if((((caster && get_turf(caster)) != last_loc) || caster.incapacitated()) && firing)
		to_chat(caster, "<span class='warning'>Your channeling is interrupted!</span>")
		stop_firing()
		return
	current_firing--
	if(current_firing <= 0)
		stop_firing()
	if(!leading_edge)
		leading_edge = fire_beam_segment(beam_icon_leading)
		leading_edge.target_scan(beam_size)
		return
	if(trailing_edge)
	...
	for(var/obj/item/projectile/magic/effect_beam/B in beam_segments)
		if(!B)
			beam_segments -= B
			continue
		B.target_scan(beam_size)

/obj/effect/proc_holder/spell/laser/proc/fire_beam_segment(icon_state_new)
	var/obj/item/projectile/magic/effect_beam/P = new /obj/item/projectile/magic/effect_beam(get_turf(intercepted_shooter))
	beam_segments += P
	P.host = src
	P.current = get_turf(intercepted_shooter)
	P.preparePixelProjectile(intercepted_target, get_turf(intercepted_target), intercepted_shooter, intercepted_params)
...
/obj/effect/proc_holder/spell/laser/InterceptClickOn(mob/living/caller, params, atom/target)
	...
	var/passlist = list()
	passlist += target
	intercepted_shooter = caller
	intercepted_params = params
	intercepted_target = target
...
/obj/effect/proc_holder/spell/laser/proc/handle_damage()
	...
	for(var/atom/A in current)
		if((A == intercepted_shooter)||(A == get_turf(intercepted_shooter)))
			continue
...
/obj/item/projectile/magic/effect_beam/proc/target_scan(scan_range)
	if(!host)
		qdel(src)
	for(var/atom/A in range(scan_range, get_turf(src)))
		if(A == host.intercepted_shooter)
			continue
		host.affected[A] = A
...
/obj/item/projectile/magic/effect_beam/on_hit(atom/target, blocked = 0)
	if(istype(target, /turf/closed) && host)

...
\
RESERVED CHARACTERS
; - Delimits multiple queries
# - Begins a command string

[comments]
(optionals)
FOR type/path, type/path (IN type/path [or] container) (WHERE args): #COMMAND args #OTHER_COMMAND other_args (; other query)

COMMANDS
SELECT - selects and enters into window and displays window. Can only have one of these per query!
CALL call_function(args), call_function_2(args_2) - Calls these functions. DOES NOT CHECK IF FUNCTION EXISTS.
UPDATE var1 = value1, val2 = value2 - Edits those variables.
DELETE - Qdels these things.

ARGS
You can include functions like so: function(args) in which case those funtions will be executed and their return values used for whatever you included them in.
Lists can be defined by [thing1, thing2]
Strings can be defined by "this_is_a_string"
Numbers can be defined by just putting them in
Associative lists can be defined by {thing1 = value1, thing2 = value2}
References can be parsed by {0x1234asdf}
usr will translate to the mob of the admin executing in
marked will translate to the admin's marked datum
GLOB will translate to the global variables holder. This can only be used with accesses.
src will translate to the current thing being ran on.
use . operator to access vars and functions of something. This will result in runtimes if it doesn't exist.

*/

#define SDQL3_STATE_UNINITIALIZED "UNINITIALIZED"
#define SDQL3_STATE_INITIALIZED "INITIALIZED"
#define SDQL3_STATE_STARTING "STARTING"
#define SDQL3_STATE_DESTROYING "GARBAGE COLLECTING"

#define SDQL3_PARSE_NOT_STARTED 0
#define SDQL3_PARSE_

#define SDQL3_QUERY_DELIMITER ";"
#define SDQL3_ESCAPE_CHARACTER "\\"

GLOBAL_LIST_INIT(SDQL3_queries, list())

/world/proc/SDQL3_query_raw(raw_text)			//Difference is this will separate delimited queries.
	var/list/separated = splittext(raw_text, regex("\[\\\]"))
	. = list()
	for(var/i in separated)
		if(!length(i))
			to_chat(world, "DEBUG: 0 length skipped.")
			continue
		. += SDQL3_query(i)

/world/proc/SDQL3_query(text)
	var/datum/SDQL3/D = new(text, usr)
	log_admin("SDQL3 query \"[text]\" triggered by [key_name_admin(usr)]")
	message_admins("SDQL3 query \"[text]\" triggered by [key_name_admin(usr)]")
	. = D
	D._start()

/datum/SDQL3
	var/query_string
	var/parse_phase = SDQL_PARSE_NOT_STARTED
	var/userkey
	var/state = SDQL3_STATE_UNINITIALIZED
	var/list/datum/selected = list()				//Everything fetched to be operated on.

/datum/SDQL3/New(string, mob/user)
	query_string = string
	userkey = user.ckey
	state = SDQL3_STATE_INITIALIZED
	GLOB.SDQL3_queries += src

/datum/SDQL3/Destroy()
	state = SDQL3_STATE_DESTROYING
	GLOB.SDQL3_queries -= src
	return ..()

/datum/SDQL3/proc/_start()
	set waitfor = FALSE
	state = SDQL3_STATE_STARTING
	Execute()
